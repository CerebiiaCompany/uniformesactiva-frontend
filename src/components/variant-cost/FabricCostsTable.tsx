...
const formatCurrency = (val: string | number) => {
    ...
    return isNaN(num) ? "0" : currencyFormatter.format(num);
};
...
export function FabricCostsTable({ data, variantId, proveedores, onAdd, onCreateProveedor }: FabricTableProps) {
    ...
    const handleConfirmAdd = async () => {
        ...
        const success = await onAdd({
            ...
            // Limpiamos los valores de puntos/comas antes de enviar a BD
            meters: newRow.meters.replace(/\./g, "").replace(",", "."),
            price_per_meter: newRow.price_per_meter.replace(/\./g, ""),
            tiene_iva: newRow.tiene_iva,
        });

        if (success) {
            setIsAdding(false);
            setNewRow({ proveedor_id: "", reference: "", meters: "", price_per_meter: "", tiene_iva: false });
        }
    };

    const calcTotal = (meters: string, price: string, iva: boolean) => {
        const m = parseFloat(meters.replace(/\./g, "").replace(",", ".")) || 0;
            ...
            <CardContent>
                ...
                {rows.map((item) => (
                    <div key={item.id} className="grid grid-cols-6 gap-2 items-center border-b py-2 text-sm">
                        <div className="truncate">{item.proveedor_nombre || proveedorNameById.get(item.proveedor_id) || "—"}</div>
                        <div>{item.reference}</div>
                        <div>{item.meters?.replace(".", ",")}</div>
                ...
                {isAdding && (
                    <div className="grid grid-cols-6 gap-2 items-center border-b py-2 text-sm bg-muted/30 rounded-lg mt-2 px-2">
                        ...
                        <Input value={newRow.reference} onChange={(e) => setNewRow({ ...newRow, reference: e.target.value })} placeholder="REF-001" />
                        <Input value={newRow.meters} onChange={(e) => setNewRow({ ...newRow, meters: e.target.value })} placeholder="0,00" />
                        <Input
                            value={
                                newRow.price_per_meter
                                    ? `$${new Intl.NumberFormat("es-CO").format(parseFloat(newRow.price_per_meter.replace(/\./g, "")))}`
                                    : ""
                            }
                            onChange={(e) => {
                                const rawValue = e.target.value.replace(/[^0-9]/g, "");
                                setNewRow({ ...newRow, price_per_meter: rawValue });
                            }}
                            placeholder="$0"
                        />
                        <Switch checked={newRow.tiene_iva} onCheckedChange={(val) => setNewRow({ ...newRow, tiene_iva: val })} />

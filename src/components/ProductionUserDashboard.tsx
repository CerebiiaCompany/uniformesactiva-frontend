...
export function ProductionUserDashboard() {
  const storedUser = useMemo(() => readStoredProductionUser(), []);
  ...
  const load = useCallback(async () => {
    let user = readStoredProductionUser();
    ...
    if (!user?.id) {
      setError("No se encontró la sesión del usuario de producción. Vuelve a iniciar sesión.");
      setLoading(false);
      return;
    }
    ...
    setError(null);
    try {
      const [orders, etapasList] = await Promise.all([fetchAllOrders(), fetchEtapas()]);
      ...
      setError(
        err instanceof Error ? err.message : "No se pudo cargar el panel de producción."
      );
    } finally {
      setLoading(false);
    }
  ...
  if (loading && !panel) {
    return (
      <AppLayout title="Mi panel producción" subtitle="Resumen de tus pedidos asignados">
  ...
  return (
    <AppLayout
      title="Mi panel producción"
      subtitle="Pedidos asignados a ti en Fábrica"
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            subtitle="Mano de obra por liquidar"
            icon={Wallet}
            variant="destructive"
          />
          <StatCard
            title="Órdenes en trabajo"
            value={data.ordersInWork}
            subtitle="Responsable actual"
            icon={Scissors}
            variant="success"
          />
        </div>
                ...
                <div className="divide-y divide-border">
                  {data.orders.map((order) => (
                    <div key={order.orderId} className="px-6 py-3 space-y-2.5">
                      ...
                      {(order.stagePayments || []).length > 0 ? (
                        <div className="rounded-lg border bg-muted/20 divide-y">
                          {order.stagePayments.map((stage) => (
                            <div
                              key={`${order.orderId}-${stage.stageKey}`}
                            ...
                            >
                              <div className="min-w-0 flex items-center gap-2 flex-wrap">
                                ...
                                {stage.inWork ? (
                                  <span className="inline-flex rounded-full bg-sky-100 text-sky-800 px-1.5 py-0.5 text-[9px] font-medium">
                                    Actual
                                  </span>
                                ) : null}
                                <span
